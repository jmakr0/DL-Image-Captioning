...
from keras.layers import Multiply, Concatenate, Reshape, Lambda, Dropout, Dense
...
def image_captioning_model_softmax(img_shape=(224, 224, 3), cnn='resnet152', embedding_dim=50, max_caption_length=15,
    ...
    embd_word_to_word = Dense(dictionary_size, activation='softmax')
    ...
    reshape_word_for_concat = Reshape((1, dictionary_size))
    ...
    for i in range(max_caption_length):
        ...
        word = embd_word_to_word(embd_word)
        words.append(reshape_word_for_concat(word))
    ...
    return C.create_compile_model([cnn_input, caption_input], caption, gpus=gpus, lr=lr,
